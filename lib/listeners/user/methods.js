...
const passwordRecoveryModel = require('lib/models/password_recovery');
...
exports.list = (meta, superAdmin) => {
	...
	let query = userModel.forge().where({
		super_admin: superAdmin
	}).orderBy(meta.order_by, meta.dir);
...
exports.saveRecoveryToken = (email, token) => {
	return userModel.where({
		email: email
	}).count().then((c) => {
		if (c === 1) {
			return passwordRecoveryModel.forge({
				email: email,
				token: token,
			}).save();
		} else {
			return new Promise((resolve, reject) => {
				reject(new hyperError.NotFound('Inputed email was not found.'));
			});
		}
	})
};

exports.getRecoveryToken = (token) => {
	return passwordRecoveryModel.where({
		token: token
	}).fetch()
};

exports.deleteRecoveryToken = (token) => {
	return passwordRecoveryModel.forge().where({
		token: token
	}).destroy({
		require: true,
	});
...
exports.updatePassword = (user, password) => {
	let newUser = {
		password: password
	};
