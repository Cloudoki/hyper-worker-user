...
const dbUsers = require('lib/listeners/user/methods')
...
validate.token = (token, accessToken) => {
	...
	if (token.userID != null) {
		return dbUsers.find(token.userID)
			.then(user => user)
	}
	// token is a client token
	return dbClients.findByClientID(token.clientID)
		.then(client => client)
}
...
validate.authCode = (code, authCode, clientID, redirectURI) => {
	...
	if( hyperError.isHyperError(verification) ) {
		return verification
	}
