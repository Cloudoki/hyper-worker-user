...
/**
* Validate if authorized by checking access token and code tables
...
exports.authorized = function (clientID, userID) {
	Promise.all([
		dbMethods.accessTokens.hasClientUser(clientID, userID),
		dbMethods.authorizationCodes.hasClientUser(clientID, userID)
	]).then((values) => {
		return new Promise((resolve, reject) => {
...
exports.trustedClient = function (clientID) {
	dbClients.findByClientID(clientID)
	.then((client) => {
		return new Promise((resolve, reject) => {
			if (client) {
				resolve(client.trusted != null)
				return
			}
...
exports.token = function (userID, accountID) {
	const token = utils.createToken({
		sub: userID,
		exp: config.oauth2.loginToken_expiresIn
	})
...
exports.client_credencials = function (client, scope, grant) {
	...
	dbClients.findByClientID(client.clientID)
		.then((localClient) => {
			return new Promise((resolve, reject) => {
			...
			return new Promise((resolve, reject) => {
				if (localClient) {
					const code = utils.createToken({
						sub: userID,
						exp: config.oauth2.code_expiresIn
					})
